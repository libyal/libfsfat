      ...
      FILE *stream )
{
	...
	fprintf( stream, "Usage: fsfatinfo [ -B bodyfile ] [ -E inode_number ] [ -F file_entry ]\n"
...
#endif
{
	...
	if( option_mode == FSFATINFO_MODE_FILE_ENTRY_BY_IDENTIFIER )
	{
		...
		if( ( string_length == 3 )
		 && ( system_string_compare(
		       ...
		       3 ) == 0 ) )
		{
			option_mode = FSFATINFO_MODE_FILE_ENTRIES;
		}
	...
	switch( option_mode )
	{
		case FSFATINFO_MODE_FILE_ENTRIES:
