...
#endif
{
	const char *description = \
		"Use fsfatmount to mount a File Allocation Table (FAT) file system volume.";

	fsfattools_option_t options[ ] = {
		{ 'h', NULL, "shows this help" },
		{ 'o', "offset", "specify the volume offset in bytes" },
		{ 'v', NULL, "verbose output to stderr, while fsfatmount will remain running in the foreground" },
		{ 'V', NULL, "print version" },
#if defined( HAVE_LIBFUSE ) || defined( HAVE_LIBFUSE3 ) || defined( HAVE_LIBOSXFUSE )
		{ 'X', "extended_options", "extended options to pass to sub system" },
#endif
		{ 0, "volume", "a File Allocation Table (FAT) file system volume" },
	...
	fsfatmount_dokan_options.SingleThread = TRUE;
#else
