     ...
     size_t size )
{
	...
	{
		if( libfsfat_extended_attribute_get_size(
		     extended_attribute,
		     &value_data_size,
		     &error ) != 1 )
		{
		...
		else
		{
			...
			read_count = libfsfat_extended_attribute_read_buffer_at_offset(
			              extended_attribute,
			              (void *) value,
			              size,
			              0,
	...
	if( libfsfat_extended_attribute_free(
	     &extended_attribute,
	     &error ) != 1 )
	{
		libcerror_error_set(
     ...
     size_t size )
{
	...
	if( libfsfat_file_entry_get_number_of_extended_attributes(
	     file_entry->fsfat_file_entry,
	...
	{
		if( libfsfat_file_entry_get_extended_attribute_by_index(
		     file_entry->fsfat_file_entry,
		     extended_attribute_index,
		     &extended_attribute,
		     &error ) != 1 )
		...
		if( libfsfat_extended_attribute_get_utf8_name_size(
		     extended_attribute,
		...
		if( size > 0 )
		{
			...
			if( libfsfat_extended_attribute_get_utf8_name(
			     extended_attribute,
...
int mount_fuse_readdir(
     ...
     void *buffer,
     fuse_fill_dir_t filler,
     off_t offset FSFATTOOLS_ATTRIBUTE_UNUSED,
     struct fuse_file_info *file_info FSFATTOOLS_ATTRIBUTE_UNUSED )
{
	...
	FSFATTOOLS_UNREFERENCED_PARAMETER( offset )
...
void mount_fuse_destroy(
      void *private_data FSFATTOOLS_ATTRIBUTE_UNUSED )
{
	libcerror_error_t *error = NULL;
	static char *function    = "mount_fuse_destroy";

	FSFATTOOLS_UNREFERENCED_PARAMETER( private_data )
