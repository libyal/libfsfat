...
#include "libfsfat_chs_address.h"
...
{
	static char *function = "libfsfat_chs_address_initialize";
	...
	*chs_address = memory_allocate_structure(
	...
	if( memory_set(
	     *chs_address,
	     0,
	     sizeof( libfsfat_chs_address_t ) ) == NULL )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_MEMORY,
...
int libfsfat_chs_address_free(
     ...
     libcerror_error_t **error )
{
	static char *function = "libfsfat_chs_address_free";
...
int libfsfat_chs_address_copy_from_byte_stream(
     ...
     libcerror_error_t **error )
{
	static char *function = "libfsfat_chs_address_copy_from_byte_stream";
	...
	if( ( byte_order != LIBFSFAT_ENDIAN_BIG )
	 && ( byte_order != LIBFSFAT_ENDIAN_LITTLE ) )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_ARGUMENTS,
	...
	if( byte_order == LIBFSFAT_ENDIAN_LITTLE )
	{
	...
	else if( byte_order == LIBFSFAT_ENDIAN_BIG )
	{
