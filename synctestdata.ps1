...
If (-Not (Test-Path "${TestsInputDirectory}\.fsfatinfo_fs"))
{
	New-Item -Name "${TestsInputDirectory}\.fsfatinfo_fs" -ItemType "directory" | Out-Null
	Write-Output "-H" | Out-File -Encoding ascii -FilePath "${TestsInputDirectory}\.fsfatinfo_fs\options"
}
