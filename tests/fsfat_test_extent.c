...
/* Tests the libfsfat_extent_clone function
 * Returns 1 if successful or 0 if not
 */
int fsfat_test_extent_clone(
     void )
{
	libcerror_error_t *error              = NULL;
	libfsfat_extent_t *destination_extent = NULL;
	libfsfat_extent_t *source_extent      = NULL;
	int result                            = 0;
	...
	destination_extent = (libfsfat_extent_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfsfat_extent_clone with malloc failing
		 */
		fsfat_test_malloc_attempts_before_fail = test_number;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfsfat_extent_clone with memcpy failing
		 */
...
/* Tests the libfsfat_extent_read_data function
 * Returns 1 if successful or 0 if not
 */
int fsfat_test_extent_read_data(
     void )
{
	libcerror_error_t *error  = NULL;
	libfsfat_extent_t *extent = NULL;
	...
	/* Test regular cases
	 */
	result = libfsfat_extent_read_data(
	          extent,
	          fsfat_test_extent_data1,
	          12,
	          &error );
...
#endif
{
	...
	FSFAT_TEST_RUN(
	 "libfsfat_extent_clone",
	 fsfat_test_extent_clone );

	FSFAT_TEST_RUN(
	 "libfsfat_extent_read_data",
	 fsfat_test_extent_read_data );
