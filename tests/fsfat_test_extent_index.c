...
#include "../libfsfat/libfsfat_extent_index.h"

#if defined( __GNUC__ ) && !defined( LIBFSFAT_DLL_IMPORT )

/* Tests the libfsfat_extent_index_initialize function
 * Returns 1 if successful or 0 if not
 */
int fsfat_test_extent_index_initialize(
     void )
{
	libcerror_error_t *error              = NULL;
	libfsfat_extent_index_t *extent_index = NULL;
	int result                            = 0;
	...
	/* Test regular cases
	 */
	result = libfsfat_extent_index_initialize(
	          &extent_index,
	          &error );
	...
	result = libfsfat_extent_index_free(
	          &extent_index,
	          &error );
	...
	extent_index = (libfsfat_extent_index_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfsfat_extent_index_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfsfat_extent_index_initialize with memset failing
		 */
...
/* Tests the libfsfat_extent_index_free function
 * Returns 1 if successful or 0 if not
 */
int fsfat_test_extent_index_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	FSFAT_TEST_RUN(
	 "libfsfat_extent_index_initialize",
	 fsfat_test_extent_index_initialize );

	FSFAT_TEST_RUN(
	 "libfsfat_extent_index_free",
	 fsfat_test_extent_index_free );

	/* TODO: add tests for libfsfat_extent_index_read_data */
