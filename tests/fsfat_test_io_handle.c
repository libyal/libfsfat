...
#include "../libfsfat/libfsfat_io_handle.h"

#if defined( __GNUC__ ) && !defined( LIBFSFAT_DLL_IMPORT )

/* Tests the libfsfat_io_handle_initialize function
 * Returns 1 if successful or 0 if not
 */
int fsfat_test_io_handle_initialize(
     void )
{
	...
	io_handle = (libfsfat_io_handle_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfsfat_io_handle_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfsfat_io_handle_initialize with memset failing
		 */
...
/* Tests the libfsfat_io_handle_free function
 * Returns 1 if successful or 0 if not
 */
int fsfat_test_io_handle_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
/* Tests the libfsfat_io_handle_clear function
 * Returns 1 if successful or 0 if not
 */
int fsfat_test_io_handle_clear(
     void )
{
	libcerror_error_t *error        = NULL;
	libfsfat_io_handle_t *io_handle = NULL;
	...
	/* Test regular cases
	 */
	result = libfsfat_io_handle_clear(
	          io_handle,
	          &error );
	...
	/* Test libfsfat_io_handle_clear with memset failing
	 */
	fsfat_test_memset_attempts_before_fail = 0;
...
#endif
{
	...
	FSFAT_TEST_RUN(
	 "libfsfat_io_handle_initialize",
	 fsfat_test_io_handle_initialize );

	FSFAT_TEST_RUN(
	 "libfsfat_io_handle_free",
	 fsfat_test_io_handle_free );

	FSFAT_TEST_RUN(
	 "libfsfat_io_handle_clear",
	 fsfat_test_io_handle_clear );
