...
Foreach (${TestName} in ${ToolsTests} -split " ")
{
	...
	$ResultRun = RunTestBinary ${TestExecutablesDirectory} "fsfat_test_tools_${TestName}"
...
For ($ProfileIndex = 0; $ProfileIndex -le ($Profiles.length - 1); $ProfileIndex += 1)
{
	...
	{
		$ResultRun = RunToolsBinaryAndCompareStdout ${TestExecutablesDirectory} "fsfatinfo" ${TestProfile} "" ${TestInput}
